...
def create_dataloader(
    ...
    # Make sure only the first process in DDP process the gun_data first, and the following others can use the cache
    ...
    # Use torch.utils.data.DataLoader() if gun_data.properties will update during training else InfiniteDataLoader()
    dataloader = loader(
        dataset,
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        return path, img, img0, self.cap, self.nframes
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        # Cache gun_data labels, check images and read shapes
        x = {}  # dict
...
def load_image(self, index):
    # loads 1 image from gun_data, returns img, original hw, resized hw
    img = self.imgs[index]
...
def extract_boxes(
    ...
    # Convert detection gun_data into classification gun_data, with one directory per class
...
def autosplit(
    ...
    """Autosplit a gun_data into train/val/test splits and save path/autosplit_*.txt files
    # Arguments
